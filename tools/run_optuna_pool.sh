...
ROUNDS="${ROUNDS:-333}"
TRIALS="${TRIALS:-100}"
STORAGE="${STORAGE:-sqlite:///tuning/optuna_all.db}"
...
IFS=',' read -ra _batches <<< "${BATCHES}"
for _batch in "${_batches[@]}"; do
  _batch="$(echo "${_batch}" | xargs)"
  if [[ -z "${_batch}" ]]; then
    continue
  fi
  "${BOT_PYTHON}" tools/optuna_tune_all.py \
    --bot-a "${BOT_A}" \
    --bot-b "${BOT_B}" \
